    ...
    : WidgetWithBackground(parent)
{
    ...
    m_dateFont.setPointSize(8);
    m_timeFont.setPointSize(30);
    m_dayFont.setPointSize(8);
    m_LogoFont.setPointSize(12);
...
{
    int side = qMin(width()/1.2, height()/1.2);
...
void WallClock::paintBackground(QPainter & painter)
{
    ...
    //QColor ramka(17,50,214);// 砤dny niebieski
    QColor ramka(215,0,0); // 红色
    // Gradient  zewn阾rznego okr阦u
    QRadialGradient back1(QPointF(0,0),135, QPointF(-27.5,110.0));
    back1.setColorAt(0.0,QColor(255,255,255));
    back1.setColorAt(1.0,ramka);    // 白色
    // Gradient wewn阾rznego okr阦u
    ...
    painter.drawEllipse(-132,-132,260,260);
// 
...
void WallClock::paintEvent(QPaintEvent *)
{
    ...
    if (logoOffset() != 0)
    {
        ...
        Str = QObject::tr("VAZYME");
        Size = painter.fontMetrics().size(Qt::TextSingleLine, Str);
        painter.drawText(QPointF(Size.width() / -2.0, static_cast<int>(0 - logoOffset())), Str);
    }

    // rysowanie wskaz體ki godzin
    painter.setPen(Qt::NoPen);
    painter.setBrush(Qt::black);
    painter.save();
